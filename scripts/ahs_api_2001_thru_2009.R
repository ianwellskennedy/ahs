...
# Create a list of all 2001-2009 household datasets
household_list <- list(household_2001, household_2003, household_2005, household_2007, household_2009)
# Create a list of all 2001-2009 project datasets
project_list <- list(project_2001, project_2003, project_2005, project_2007, project_2009)
...
# Finalize the 2001-2009 data by joining amended household/project datasets
...
rm(household_variables, project_variables, household_list, project_list, selected_datasets, i, select_columns, years, packages, install_if_missing)
...
reclassify <- function(data) {
  ...
  data <- data %>%
    ...
    # Edit the CSA variable to read 'Others' for those in 2001-2009 but not in 2015-2023
    ...
    # SINHV = VACINHER (2023)
    mutate(SINHV = case_when(SINHV == 'X' ~ 'Yes',
                             SINHV == ' ' ~ 'NR',
                             SINHV == '-6' ~ NA)) %>%

    # Amend the SINVV variable (-6 here is NA, -9 here is 'Not Reported')
    # SINVV = VACINVEST (2023)
    mutate(SINVV = case_when(SINVV == 'X' ~ 'Yes',
                             SINVV == ' ' ~ 'NR',
                             SINVV == '-6' ~ NA)) %>%
    ...
    rename(BLD = NUNIT2, HINCP = ZINC2, HOWBUY = CUSHOM, UNITSIZE = UNITSF, LOTSIZE = LOT, INTSTATUS = ISTATUS,
           ...
           YRBUILT = BUILT, JOBTYPE = RAS, JOBDIY = RAH, JOBCOST = RAD, VACINHER = SINHV, VACINVEST = SINVV)
}

# Read in the 2011-2023 data and store as data_2011_2023
data_2011_2023 <- read.xlsx(data_2011_2023_file_path)
...
data_2001_2009 <- data_2001 %>%
  # Attach 2003-2009 datasets to 2001 by row
...
# Apply replace_non_response_values_2() to the 2001-2009 data, this will replace any 'NR' values with -1
data_2001_2009 <- replace_non_response_values_2(data_2001_2009)
...
#Read in column names from 2011-2023 cleaned data
col_order_check <- names(data_2011_2023)
#Specify the column order for 2001-2009
col_order <- col_order_check[col_order_check %in% names(data_2001_2009)]
#Find columns missing from 2001-2009 though present in 2011-2023
missing_cols <- col_order_check[!col_order_check %in% col_order]

# Reorder 2001-2009 data to match the column order in 2011-2023
data_2001_2009 <- data_2001_2009[col_order]

# Bind the 2001-2009 data to the 2011-2023 data. rbind.fill() fills all columns from 2001-2009 that are present in 2011-2023 (binding them as new rows)
# 2001-2009 data will have NA values for all of the 'missing_cols'
data_2001_2023 <- data_2011_2023 %>%
  rbind.fill(data_2001_2009)

# Order the 2001-2023 data by year
data_2001_2023 <- data_2001_2023[order(data_2001_2023$AHSYEAR), ]
